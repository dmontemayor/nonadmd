...
!----------------------------------------------------------------
  subroutine display_CRYST1(this,unit)
    ...
    if(ierr.EQ.1)call warn('WRITE_CRYST1: could not write record!')
...
!----------------------------------------------------------------
  subroutine display_MODEL(this,unit)
    ...
    if(ierr.EQ.1)call warn('WRITE_MODEL: could not write record!')
...
!----------------------------------------------------------------
  subroutine display_ATOM(this,unit)
    ...
    if(ierr.EQ.1)call warn('WRITE_ATOM: could not write record!')
...
!>\details reads the next chunk of the pdb file.
  subroutine refresh_buffer
    ...
    do irecord=1,maxrecord
       ...
       if(ierr.EQ.0)buffermask(irecord)=.true.
    end do
...
!<------------------------------------------
  ...
  subroutine display_psfatom(this,unit)
    ...
    if(ierr.EQ.1)call warn('display: could not display psf atom!')
...
!>\details reads the next chunk of the psf file.
  subroutine refresh_psfbuffer
    ...
    do irecord=1,maxrecord
       ...
       if(ierr.EQ.0)psfbuffermask(irecord)=.true.
    end do
  ...
  subroutine advance_psf(section,nrecord)
    ...
    if(psfisopen)then
       ...
       if(present(section))then
          ...
          case default
             call warn('advance_psf: unknown section! Valid options are'&
                  //': title, atoms, bonds, angles, dihedrals, impropers, or cross-terms'&
                  ,'ignorning advance_psf command.')
          end select
       else
