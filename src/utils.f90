...
contains
  ...
  function long2str(I)
    ...
    if((I.NE.I).or.(abs(I).GT.huge(I)))then
       ...
       if(ierr)long2str='int2str_ERROR'
    end if
  ...
  function double2str(X)
    ...
    if((X.NE.X).or.(abs(X).GT.huge(X)))then
       ...
       if(ierr)double2str='float2str_ERROR'
    end if
...
contains
  ...
  subroutine displayCMPLXMatrix(Z,tol,msg)
    ...
    write(Dunit,FMT)((textgraphsChar(real(Z(i,j)),tol=mag,alt=.false.)&
         //textgraphsChar(aimag(Z(i,j)),tol=mag,alt=.True.),j=1,M),i=1,N)
  ...
  !! %* : significant imag positive value\n
