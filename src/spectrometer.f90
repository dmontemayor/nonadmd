...
contains
  ...
  !<-------------------------------------------------------------------------------
  subroutine get_spectrum(this,qs,cs,cp,Emin,Emax,npt,type,tol,samples&
       ,file,pdb,psf,psfx,atm_select,dipole_to,dipole_from,dipole_center)
    ...
    case('AB')
       if(present(cs).and.present(cp))then
          ...
          call warn('get_spectrum: cannot compute AB spectrum, missing classical subsystem or coupling term.'&
               ,'exiting.')
       end if
    case('ABPDB')
       call get_ABPDBspectrum(this,qs,pdb=trim(pdb),psf=trim(psf),psfx=trim(psfx)&
            ,file=trim(outfile),atm_select=atm_select,dipole_to=dipole_to&
            ,dipole_from=dipole_from,dipole_center=dipole_center)
    case default
  ...
  subroutine get_ABspectrum(this,qs,cs,cp,tol,samples,file)
    ...
    real(double),allocatable::Eng(:),MM(:,:),spektrum(:)
    ...
    if(pass)then
       ...
       if(allocated(spektrum))deallocate(spektrum)
       allocate(spektrum(0:this%npt-1))
       ...
       do while(.not.cnvrg)
          do isample=lastsample+1,nsample
             ...
             do kstate=1,nstate
                ...
                do i=0,this%npt-1
                   w=Emin+i*dw
                   if(Eng(kstate).GT.w-dw/2_double.and.Eng(kstate).LE.w+dw/2_double)spektrum(i)=spektrum(i)+tr
                end do
             end do
          end do
          ...
          R2=0.0_double
          if(stable)R2=sum((spektrum/real(nsample)-this%spectrum)**2)/real(this%npt)
          ...
          this%spectrum=spektrum/real(nsample)
          lastsample=nsample
       ...
       if(.not.stable)spektrum=0.0_double !incase of runtime error
...
!!$!--------------------------------------------------------
  ...
  subroutine get_ABPDBspectrum(this,qs,file,pdb,psf,psfx&
       ,atm_select,dipole_to,dipole_from,dipole_center)
!--------------------------------------------------------------------------------
!> \brief
