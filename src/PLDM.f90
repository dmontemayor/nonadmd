...
contains
  ...
  subroutine PLDM_new(this,qs,cs,cp,file)
    ...
    if(check(qs))call stop('PLDM_new, failed qs object check.')
    if(check(cs))call stop('PLDM_new, failed cs object check.')
    if(check(cp))call stop('PLDM_new, failed cp object check.')
    ...
    if(check(this%qs))call stop('PLDM_new, failed this%qs object check.')
    if(check(this%cs))call stop('PLDM_new, failed this%cs object check.')
    if(check(this%cp))call stop('PLDM_new, failed this%cp object check.')
...
!!$    allocate(this%qt(this%qs%hs%nstate),this%pt(this%qs%hs%nstate))
    ...
    if(check(this))call stop('PLDM_new, failed final object check.')
...
!!$    if(associated(this%pt))nullify(this%pt)
  ...
  subroutine PLDM_save(this,file)
    ...
    if(check(this))call stop('PLDM_save, failed object check.')
...
!    real(double),allocatable::x0(:),p0(:),xt0(:),pt0(:)
    ...
    if(check(this))call stop('PLDM_run: input object failed check!')
...
!!$    end do
    ...
    do itraj=1,proctraj
       ...
       call Note('trajectory: '//int2str(itraj)//' initial state: '//int2str(istate)// ','//int2str(istate))
