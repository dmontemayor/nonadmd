...
contains
  ...
  subroutine harmonicbath_init(this,file)
    ...
    if(check(trim(filename)))call stop('harmonicbath_init: input file '//trim(filename)//' not found')
...
222    rewind(unit)
    ...
    call resample(this)
    if(check(this))call stop('harmonicbath_init, failed exit object check')
  ...
  subroutine harmonicbath_save(this,file)
    ...
    if(check(this).NE.0)then
       ...
       write(unit,*)(this%map(idensity),idensity=1,this%nden)
  ...
  subroutine harmonicbath_resample(this)
    ...
    call update(this)
    if(check(this))call stop('harmonicbath_resample, object failed exit check')
  ...
  integer(short) function harmonicbath_check(this)
    ...
    do ipt=this%npt,1,-1
       ...
       if(loc(1).NE.ipt)then
          call Warn('harmonicbath_check: wgrid is unordered from min to max. val='//trim(float2str(this%wgrid(ipt)))//' loc='//trim(int2str(ipt))//' maxloc='//trim(int2str(loc(1))))
          harmonicbath_check=1
          return
       end if
    end do
