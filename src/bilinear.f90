...
!Change Log:
!=== May 13 2015
! + added cterm (counterterm) attribute to bilinear class.
! - counterm nolonger shifts bath modes instead adds reorginzation energy to coupling term.
!      (July 17 2013 change was reverted to make state by state energy calculation easier though coupling term
!       alone instead of also needing the shifted classical bath terms.)
!=== July 17 2013
...
module bilinear_class
  ...
  type bilinear
     ...
     real(double),dimension(:),pointer::lambda,cterm
...
contains
  ...
  subroutine bilinear_init(this,qs,cs,file)
    ...
    if(associated(this%cterm))nullify(this%cterm)
    allocate(this%cterm(nstate),stat=ierr)
  ...
  subroutine bilinear_update(this)
    ...
    do istate=1,nstate
       ...
       this%hc%V(istate,istate)=this%cterm(istate)
       do idof=1,ndof
  ...
  subroutine bilinear_resample(this)
    ...
    !recalculate cterm if using counterterm
    this%cterm=0._double
    if(this%CTused)then
       do istate=1,nstate
          do idof=1,ndof
             if(this%gmap(idof,istate))this%cterm(istate)=this%cterm(istate)&
                  +(this%g(idof,istate)*this%f(istate)/this%hc%hb%mode(idof))**2
          end do
       end do
       this%cterm=this%cterm/real(2,double)
    end if
...
!!$    end if
  ...
  subroutine bilinear_display(this,msg)
    ...
    write(Dunit,*)'counter term  by state (bhor)=',this%cterm
  ...
  integer(short) function bilinear_check(this)
    ...
    !real(double),dimension(:),pointer::cterm
    if(.not.associated(this%cterm))then
       call Warn('bilinear_check: cterm memory not allocated')
       bilinear_check=1
       return
    end if
    if(size(this%cterm).NE.nstate)then
       call Warn('bilinear_check: size of cterm not equal number of states')
       bilinear_check=1
       return
    end if
    if(any(this%cterm.NE.this%cterm))then
       call Warn('bilinear_check: cterm has NAN values.')
       bilinear_check=1
       return
    end if
    if(any(abs(this%cterm).GT.huge(this%cterm)))then
       call Warn('bilinear_check: cterm has Huge values.')
       bilinear_check=1
       return
    end if
    if(any(this%cterm.LT.0._double))then
       call Warn('bilinear_check: cterm has negative values.')
       bilinear_check=1
       return
    end if
