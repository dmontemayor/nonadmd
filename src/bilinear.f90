...
contains
  ...
  subroutine bilinear_init(this,qs,cs,file)
    ...
    if(check(qs))call stop('bilinear_init, qs failed check')
    if(check(cs))call stop('bilinear_init, cs failed check')
    ...
    if(check(this%hc))call stop('bilinear_init: coupling primitive failed check')    
    ...
    if(check(this))call stop('bilinear_init: failed final object check')
  ...
  subroutine bilinear_resample(this)
    ...
    if(check(this))call stop('bilinear_resample: object failed exit check.')    
  ...
  subroutine bilinear_save(this,file)
    ...
    if(check(this))call stop('bilinear_save')
  ...
  subroutine bilinear_display(this,msg)
    ...
    do istate=1,nstate
       ...
       call display(this%hc%hb%mode/invcm,this%g(:,istate)*angstrom/invcm,mask=mask,msg='Coupling constant plot: X(1/cm),Y(hbar/(cm*Angstrom))')
    end do
  ...
  integer(short) function bilinear_check(this)
    ...
    if(any(abs(this%lambda).LT.epsilon(this%lambda)))then
       call Warn('bilinear_check: lambda has tiny values.')
       bilinear_check=1
       return
    end if
    ...
    if(any(this%bdof.LE.0))then
       call Warn('bilinear_check: bdof must be positive.')
       bilinear_check=1
       return
    end if
