...
subroutine JSB850(file)
  ...
  integer(long),parameter::Rstate=18,shift=9
     ...
     do jstate=istate+1,Rstate
        ...
        if(donor(1:1).NE.acceptor(1:1))then
           if(Vvart.LE.Vvarmin)then
              Vvarmin=Vvart
              call Note('JSB850: min pair ('//donor//','//acceptor//') distance in angstrom ='//trim(float2str(R/angstrom)))
           end if
           if(Vvart.GE.Vvar0)then
              Vvar0=Vvart
              call Note('JSB850: max pair ('//donor//','//acceptor//') distance in angstrom ='//trim(float2str(R/angstrom)))
           end if
        end if
