...
subroutine LH2(file)
  ...
  integer(long),parameter::Rstate=27,shift=0
  integer(long)::map(Rstate)
  !                                                            !Krueger !JS
  real(double),parameter::trans          =78.0_double*angstrom !NA(78)  !NA(78)
  real(double),parameter::B800B800_cutoff=32.0_double*angstrom !32      !12
  real(double),parameter::B800B850_cutoff=24.0_double*angstrom !22      !12
  ...
  real(double),parameter::thetaD=90.0_double*Deg !JS
  !real(double),parameter::thetaD=0.0_double*Deg
  ...
  real(double),parameter::En=invcm*1E7/831.0_double !812.0 JS
  !real(double),parameter::En=invcm*1E7/832.0_double !with counterterm

  real(double),parameter::bias=480*invcm !260.0_double*invcm JS
  !real(double),parameter::bias=500*invcm !with counterterm

  !real(double),parameter::sigEg=40.0_double*invcm    !external site disorder JS
  real(double),parameter::sigEg=40.0_double*invcm    !external site disorder

  !real(double),parameter::sigEDg=54.0_double*invcm    !B800 internal site disorder (from JS ref)
  real(double),parameter::sigEDg=54.0_double*invcm    !B800 internal site disorder 

  real(double),parameter::sigEng=165.00_double*invcm  !B850 internal site disorder [sqrt(sigEn**2-sigEg**2)]
  !real(double),parameter::sigEng=246.78_double*invcm !B850 internal site disorder [sqrt(sigEn**2-sigEg**2)] JS
     ...
     do jstate=istate+1,Rstate
        ...
        if(donor(1:1).NE.acceptor(1:1))then
           if(Vvart.LE.Vvarmin)then
              Vvarmin=Vvart
              call Note('LH2: min pair ('//donor//','//acceptor//') distance in angstrom ='//trim(float2str(R/angstrom)))
           end if
           if(Vvart.GE.Vvar0)then
              Vvar0=Vvart
              call Note('LH2: max pair ('//donor//','//acceptor//') distance in angstrom ='//trim(float2str(R/angstrom)))
           end if
        end if
