...
!==============================
!Updates:
!(*) Convert this subroutine to explicitly generate Jang and Silbey model
!   (&*) Reverted Bias to 260 wavenumbers
!   (&*) State energy fluctuation is now in terms of internal disorder 
!                 (ground to excited state gap fluctuation) -> total fluctuation = internal+external disorder
!   (&*) Ground state fluctuation is the definition of external disorder
!------------v1.3 Dec 2011
...
!==============================
!Observations:
!(?)B800 dipole vectors point normal to the BCL face not along the face.
...
subroutine JSLH2(file)
  ...
  integer(long),parameter::Rstate=19,shift=8
  ...
  real(double),parameter::thetaD=90.0_double*Deg !defined by Jand and Silbey
  ...
  real(double),parameter::Eg=0.0 !ground state energy
 
  !real(double),parameter::En=0.0_double*invcm
  real(double),parameter::En=invcm*1E7/812.0_double !B850 average site energy relative to Eg

  real(double),parameter::bias=260.0_double*invcm   !gap between B800 and B850 site energy  
...
!!$  real(double),parameter::sigED=sqrt(sigEDg**2+sigEg**2) !from convolution  =Total B800 site energy fluct
  ...
  do istate=1,nD
     do jstate=nD+1,3*nD
        optdielect(istate,jstate)=(MuD/(5.3_double*Debye))**2!1.343 from Jang and Silbey 2007
     end do
  ...
  do istate=1,nD
     do jstate=nD*3+1,nD*4
        optdielect(istate,jstate)=1E3 !make huge so the two monomers don't communicate
     end do
  end do
  !B800 with B850
  do istate=1,nD
  ...
  !copy values to from monomer 1 to monomer 2
  ...
  !save quantum subsystem primitive (labeled hs)
  unit=newunit()
  ...
  !------make dipole quantum subsystem (hs + dipole information)
  do istate=1,nD
  ...
  !translate transition dipoles, coordinates and energies onto second LHII monomer
  do istate=1,nstate/2
  ...
  qs%Evar=0._double   !variance in site energy gaps
  qs%Gvar=0._double   !variance in ground state energy
  call resample(qs)

  !make corrections to dipole-dipole coupling according to Krueger and save
  !(Jang and Silbey describe their corrections in the 2007 paper)
  !B800-B800 states
  do istate=1,nD
     do jstate=1,nD
        !construct ring1
  ...
  qs%Gvar=sigEg*sigEg !when using external disorder
  Evar=0.0_double
  do istate=1,nD
  ...
  !write out a log file with transition dipole positions, orientations, and coupling energies.
  unit=newunit()
     ...
     do jstate=istate+1,Rstate
        ...
        if(donor(1:1).NE.acceptor(1:1))then
           if(Vvart.LE.Vvarmin)then
              Vvarmin=Vvart
              call Note('JSLH2: min pair ('//donor//','//acceptor//') distance in angstrom ='//trim(float2str(R/angstrom)))
           end if
           if(Vvart.GE.Vvar0)then
              Vvar0=Vvart
              call Note('JSLH2: max pair ('//donor//','//acceptor//') distance in angstrom ='//trim(float2str(R/angstrom)))
           end if
        end if
